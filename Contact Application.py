...
class Contact:
    def __init__(self,root):
        ...
        lbl_footer = Label(self.root, text="Developed by Nikul Jadhao and Bhushan Manapure", font=("times new roman", 15, "bold"),
        ...
        Btn_Frame.place(x=9,y=530,width=450)
        addbtn=Button(Btn_Frame,text="Add",font=("times new roman",12,"bold"),width=10,bg="green",fg="white",
                      command=self.add_contacts).grid(row=0,column=0,padx=5,pady=5)
        updatebtn=Button(Btn_Frame,text="Update",font=("times new roman",12,"bold"),bg="green",fg="white",width=10,
                         command=self.update_data).grid(row=0,column=1,padx=5,pady=5)

        deletebtn=Button(Btn_Frame,text="Delete",font=("times new roman",12,"bold"),bg="green",fg="white",width=10,
                         command=self.delete_data).grid(row=0,column=2,padx=5,pady=5)
        clearbtn=Button(Btn_Frame,text="Clear",font=("times new roman",12,"bold"),bg="green",fg="white",width=10,
                        command=self.clear).grid(row=0,column=3,padx=5,pady=5)
        ...
        searchbtn=Button(Details_Frame,text="Search",bg="green",fg="white",font=("times new roman",12,"bold"),
                         command=self.search_data,width=10,pady=5).grid(row=0,column=3,padx=10,pady=10)
        showallbtn=Button(Details_Frame,text="Showall",bg="green",fg="white",font=("times new roman",12,"bold"),
                          command=self.fetch_data,width=10,pady=5).grid(row=0,column=4,padx=10,pady=10)
        ...
        self.contacts_table=ttk.Treeview(Table_Frame,columns=("Name","Email","Gender","Mobile No","Tele No","DOB","Address"),
    ...
    def add_contacts(self):
        if(self.name_var.get()=="" or self.email_var.get()=="" or self.gender_var.get()=="" or self.mobileNo_var.get()=="" or
           self.dob_var.get()=="" or self.txt_address.get('1.0',END)=="" ): 
            messagebox.showerror("Error","Required fields are not filled")
        else:
            conn=connect("contacts.db")
            cur=conn.cursor()
            cur.execute("insert into contacts values(?,?,?,?,?,?,?)",(self.name_var.get(),self.email_var.get(),self.gender_var.get(),
                                                                      self.mobileNo_var.get(),self.teleNo_var.get(),self.dob_var.get(),
...
cur=con.cursor()
cur.execute("""CREATE TABLE IF NOT EXISTS contacts(Name text NOT NULL, Email text NOT NULL,Gender text NOT NULL,MobileNo integer(10) NOT NULL,
            teleNo integer(10) ,DateOfBirth text, Address text NOT NULL); """)
con.commit()
con.close()
ob = Contact(root)
root.mainloop()
